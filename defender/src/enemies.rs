...
impl Enemy {
    ...
    pub fn new_bomber(pos: Vec2, id: u32) -> Self {
        let dir = if macroquad::rand::gen_range(0u32, 2) == 0 {
            1.0
        } else {
            -1.0
        };
        Enemy {
            ...
            ai: AiState::Bomber {
                bomb_timer: BOMBER_BOMB_RATE * 0.5,
            },
            alive: true,
            id,
            angle: 0.0,
    ...
    pub fn new_pod(pos: Vec2, id: u32) -> Self {
        Enemy {
            ...
            kind: EnemyKind::Pod,
            ai: AiState::Pod {
                phase: macroquad::rand::gen_range(0.0f32, std::f32::consts::TAU),
            },
            alive: true,
            id,
            angle: 0.0,
    ...
    pub fn update(
        ...
        dt: f32,
        player_pos: Vec2,
        astronauts: &mut [Astronaut],
        bullets: &mut Vec<Bullet>,
        terrain: &Terrain,
        time: f32,
    ) {
        match self.kind {
            EnemyKind::Lander => self.update_lander(dt, astronauts, terrain),
            EnemyKind::Mutant => self.update_mutant(dt, player_pos),
            EnemyKind::Bomber => self.update_bomber(dt, bullets),
            EnemyKind::Pod => self.update_pod(dt, time),
            EnemyKind::Swarmer => self.update_swarmer(dt, player_pos),
            EnemyKind::Baiter => self.update_baiter(dt, player_pos, bullets),
        }
        self.pos.x = self.pos.x.rem_euclid(WORLD_WIDTH);
        self.angle += dt * 1.5; // gentle rotation for animated enemies
    }

    fn update_lander(&mut self, dt: f32, astronauts: &mut [Astronaut], terrain: &Terrain) {
        let mut ai = match std::mem::replace(&mut self.ai, AiState::Lander(LanderAi::Patrolling)) {
            AiState::Lander(ai) => ai,
            other => {
                self.ai = other;
                return;
            }
        ...
        let mut next_ai: Option<LanderAi> = None;
        ...
        match &mut ai {
            LanderAi::Patrolling => {
                ...
                if let Some((_, astro_id)) = best {
                    next_ai = Some(LanderAi::Descending {
                        target_id: astro_id,
                    });
                }
            }
                ...
                if let Some(target) = target {
                    if matches!(target.state, AstronautState::OnGround) {
                        ...
                        if wrap_dist_sq(self.pos, target.pos) < 20.0 * 20.0 {
                            if let Some(a) = astronauts.iter_mut().find(|a| a.id == target_id) {
                                a.state = AstronautState::BeingCarried {
                                    _by_enemy_id: self.id,
                                };
                            }
                            next_ai = Some(LanderAi::Carrying {
                                astro_id: target_id,
                            });
                        }
                    } else {
                        // Astronaut gone, resume patrol
                        next_ai = Some(LanderAi::Patrolling);
                        self.vel.x = LANDER_SPEED
                            * if macroquad::rand::gen_range(0u32, 2) == 0 {
                                1.0
                            } else {
                                -1.0
                            };
            ...
            LanderAi::Carrying { astro_id } => {
                ...
                if let Some(a) = astronauts.iter_mut().find(|a| a.id == astro_id) {
                    a.pos = Vec2::new(
                        self.pos.x,
                        self.pos.y + LANDER_HALF_H + ASTRONAUT_HALF_H + 2.0,
                    );
                }
        ...
        if matches!(ai, LanderAi::Patrolling) {
            let terrain_y = terrain.surface_y(self.pos.x);
            self.pos.y = self
                .pos
                .y
                .clamp(SCANNER_HEIGHT + 20.0, terrain_y - LANDER_HALF_H - 5.0);
        }

        self.ai = AiState::Lander(ai);
    }
    ...
    fn update_bomber(&mut self, dt: f32, bullets: &mut Vec<Bullet>) {
        ...
        self.pos.y = self.pos.y.clamp(SCANNER_HEIGHT + 20.0, WORLD_HEIGHT * 0.6);
